# Данные для языка `kum` от вендора Ali Kuzhuget
...
## stats/
Один файл kum_population.csv со столбцами:
- year
...
- kum_key_default.json

## frequencies/
kum_monocorpus_freq.csv — частотности символов.

## mapping/
kum_key_mapping.json — маппинг расширенных букв на русские клавиши.
...
extracted_letters AS (
  SELECT
    regexp_extract_all(
      norm_text,
      '(ГЪ)|(ГЬ)|(КЪ)|(НГ)|(ОЬ)|(УЬ)|([АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ])'
    ) AS letter
